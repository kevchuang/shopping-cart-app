  ...
  def make[F[_]: Sync](passwordSalt: PasswordSalt): F[Crypto] =
    Sync[F]
      .delay {
        ...
        val eCipherInstance = Cipher.getInstance("AES/CBC/PKCS5Padding")
        val eCipher         = EncryptCipher(eCipherInstance)
        eCipher.value.init(Cipher.ENCRYPT_MODE, sKeySpec, iv)
        val dCipherInstance = Cipher.getInstance("AES/CBC/PKCS5Padding")
        val dCipher         = DecryptCipher(dCipherInstance)
      ...
      .map { case (ec, dc) =>
        new Crypto:
          def encrypt(password: Password): EncryptedPassword =
            ...
            EncryptedPassword.assume(result)
